...
export default function ExpensesSection({ form }: { form: any }) {
  const expensesData = form.values.expenses || [];

  const rows = months.map((month, index) => {
    const monthNumber = index + 1;
    const rowKey = `expense-${monthNumber}`;

    const existingRow = expensesData.find(
      (r: any) =>
        r.month === monthNumber && r.year === (form.values.age || 2025)
    );

    const sumPath = `expenses.${index}.sum`;
    const typePath = `expenses.${index}.type`;
    const statusPath = `expenses.${index}.status`;
      ...
      <Table.Tr key={rowKey}>
        ...
        <Table.Td>
          <Select
            ...
            data={[
              { value: "1", label: "Персонал" },
              { value: "2", label: "Оборудование" },
              { value: "3", label: "Услуги" },
              { value: "4", label: "ПО/Лицензии" },
              { value: "5", label: "Прочее" },
            ]}
            {...form.getInputProps(typePath)}
            size="sm"
            style={{ width: "100%" }}
          />
        </Table.Td>
        <Table.Td>
          <Select
            placeholder="Выберите статус отражения"
            data={[
  ...
  return (
    <Paper radius={10} withBorder p={16} mb={20}>
      <Group justify="space-between" mb="md">
        <Text fw={500}>Информация по затратам проекта</Text>
        <Select
          ...
          <Table withRowBorders verticalSpacing="sm">
            <Table.Thead>
              <Table.Tr>
                ...
                <Table.Th>Вид затрат</Table.Th>
                <Table.Th>Статус отражения</Table.Th>
              </Table.Tr>
            </Table.Thead>
